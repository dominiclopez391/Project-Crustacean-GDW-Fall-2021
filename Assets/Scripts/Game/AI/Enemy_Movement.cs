...
public class Enemy_Movement : MonoBehaviour
{
    ...
    public Enemy_Movement Initialize(Rigidbody2D rb, Enemy en, PhysicsMaterial2D ff, PhysicsMaterial2D nf)
    {
        this.en = en;
        this.rb = rb;
        fullFriction = ff;
    ...
    {
        if (velY > en.MAX_FALL_SPEED)
        {
            velY -= en.GRAVITY_ACCELERATION * Time.deltaTime;
        }
    }
    ...
    public void UpdateWalk(float vel)
    {
        ...
        else if (vel > 0)
        {
            ...
            if (accel)
            {
                if (velX < en.WALK_MAX_SPEED)
                {
                    velX += vel * en.WALKING_ACCELERATION * Time.deltaTime;
                }
                else if (velX > en.WALK_MAX_SPEED)
                {
                    velX = en.WALK_MAX_SPEED;
                }
            }
            else
            {
        ...
        else if (vel < 0)
        {
            ...
            if (accel)
            {
                if (velX < -1 * en.WALK_MAX_SPEED)
                {
                    velX = -1 * en.WALK_MAX_SPEED;
                }
                else if (velX > -1 * en.WALK_MAX_SPEED)
                {
                    velX += vel * en.WALKING_ACCELERATION * Time.deltaTime;
                }
    ...
    {
        float speed = dash ? en.DASH_SPEED : en.WALK_MAX_SPEED;
    ...
    public void Glide()
    {
        ...
        else if (velY > -1 * en.GLIDE_SPEED)
        {
            velY += -1 * en.WALKING_ACCELERATION * Time.deltaTime;
        }
        else
        {
            velY = -1 * en.GLIDE_SPEED;
        }
    ...
    public bool CanCoyoteJump()
    {
        ...
        return CoyoteJumpTime > Time.time - en.COYOTE_JUMP_TIME;
    }
    ...
    {
        if (velY < 0 && BufferJumpTime > Time.time - en.BUFFER_JUMP_TIME)
        {
            return true;
        }
        return false;
    ...
    {
        if (Time.time > dashTime + en.DASH_DURATION)
        {
            return true;
        }
        return false;
    ...
    /**
     * It has to figure out if an obstacle wall has been hit
     * This is simple, its the inverse of figuring out if the ground has been hit
     * */
    ...
    //Exiting collision must mean enemy left ground, if on it
